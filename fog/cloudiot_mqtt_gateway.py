##### FOG DEVICE #####
# Modification of https://cloud.google.com/community/tutorials/cloud-iot-gateways-rpi
# Modified to include distance calculation and mqtt topic 
#
#
...
def main():
    ...
    while True:
        ...
        if action == 'event':
            ...
            udpSerSock.sendto(message.encode('utf8'), client_addr)
        elif action == "people_found":
            coordinates = command["data"]
            location = command["location"]
            #print(location)
            #   client.publish(subscribe_topic,"{too close}")
            for i in range(len(coordinates)-1):
                for j in range(i+1,len(coordinates)):
                    dist=calculateDistance(coordinates[i],coordinates[j])
                    print(dist)
                    if dist<500:
                        coordstr = ''.join(str(c) for c in coordinates)
                        mqtt_topic = '/devices/{}/events'.format(device_id)
                        print('Sending human event for device {}'.format(device_id))
                        mqtt_topic = '/devices/{}/events'.format(device_id)
                        print('Publishing message to topic {} with coordinates \'{}\''.format(
                                mqtt_topic, coordinates))
                        _, event_mid = client.publish(mqtt_topic, location, qos=0)
